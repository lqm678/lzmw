...
# nin.exe -------------------------------------------------
Get difference-set(not-in-later) for first file/pipe; Or intersection-set with later file. by LQM:
  ...
  -S [ --switch-first ]      Switch first/later places for files or pipe/file.
...
Useful : -H 0, -M, -S, -PAC, -i -u, -ium, -iwn, -im, -imw, -iupdw, -iupdPAC
-iwn : to get lines in first and not captured in later, But including not-captured in first, ignore case.
-ium : to get unique common intersection, ignore case.
-iup : to get distribution percentage, ignore case.
...
# lzmw.exe ------------------------------------------------
Line matching/replacing (IGNORE case of file and directory name) by LQM:
...
(4) Execute output line as command : If has -X (--execute-out-lines): 
    ...
    only for cases which lzmw cannot know where to read from : files or pipe. For a simple example : 
...
Example-7 : Display current modified code files : Now time = 2017-03-26 22:14:42.102976 CST
    for /F %s in ('lzmw -l -f "\.(cs|java|cpp|cx*|hp*|py|scala)$" -rp "%CD%" --nd "^(debug|release)$"  --w1 "2017-03-26 22:14:42" -PAC 2^>nul ') do @echo code file : %s
...
Example-9 : Find file in PATH environment variable : such as ATL*.dll : 
    for /f "tokens=*" %d in ('lzmw -z "%PATH%" -t "([^;]+);*" -o "$1\n" -PAC ^| lzmw -it "\\\s*$" -o "" -a -PAC') do @lzmw -l --wt --sz -p "%d" -f "^ATL.*\.dll$" -O 2>nul

All commands are optional and no order, but case sensitive.
Can merge single char commands/values like : -rp -it -ix -PIC -PAC -POC -PIOCcl -PICc -PICcl , -mu -v zod, -muvzod, -uvz
Useful : -H 0, -M, -O, -PAC, -PIC, -POC, -POlCc, -XI, -XIP, -XA, -XO, -XOPI, -muvz, 2>&1, 2>nul (2^>nul in pipe)
